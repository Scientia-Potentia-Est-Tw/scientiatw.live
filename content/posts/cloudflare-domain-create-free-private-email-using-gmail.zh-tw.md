...
featureimage: "https://media.secologies.com/cloudflare-domain-create-free-private-email-using-gmail.webp"
---
![-domain-create-free-private-email-using-gmail-cover](https://media.secologies.com/cloudflare-domain-create-free-private-email-using-gmail.webp)
...
![Cloudflare Email Routing頁面](https://media.secologies.com/Cloudflare-Email-Routing-Page.webp)

進到頁面後，就可以做初始設定，按下Get started

![Cloudflare Email Routing初始設定](https://media.secologies.com/Cloudflare-email-routing-getstart.webp)
...
![Cloudflare Email Routing設定電子郵件位址](https://media.secologies.com/Cloudflare-email-routing-custom-setting.webp)

Cloudflare就會寄送一封確認信到你想要轉寄的信箱裡，直接點選驗證即可

![Cloudflare Email Routing信箱驗證](https://media.secologies.com/Cloudflare-routing-verify-mail.webp)

回到Cloudflare Email Routing頁面裡，就可以在Overview看到已新增的個人紀錄，以及在Routing rules可以看到之前的設定

![Cloudflare Email Routing總覽](https://media.secologies.com/Cloudflare-email-routing-overview.webp)

![Cloudflare Email Routing規則](https://media.secologies.com/Cloudflare-email-routing-rule.webp)

另外設定完信箱之後，還需要新增DNS紀錄指定給這個網域，會出現在Overview頁面的底下，指示需要啟動下列的紀錄，基本上按下Add records and enable的就行了，不用一個一個自己新增

![Cloudflare Email Routing所需的DNS Records](https://media.secologies.com/cloudflare-email-routing-required-records.webp)
...
![建立App Password](https://media.secologies.com/Create-App-Password.webp)

按下建立之後，就會給一組密碼，請記錄下來，並且把密碼之間的**空格**刪掉，之後在新增信箱時會需要用到，回到Gmail介面當中，按下右上角的齒輪，找到所有設定，並且到"帳號及匯入"的功能裡，按下新增其他電子郵件信箱

![Gmail設定](https://media.secologies.com/gmail-see-all-settings.webp)

![Gmail 帳號及匯入功能](https://media.secologies.com/add-another-email-address-3.webp)
...
![設定客製化信箱](https://media.secologies.com/adding-another-address.webp)
...
![設定Gmail SMTP Server](https://media.secologies.com/send-mail-through-smtp-server-2.webp)

顯示成功新增後，會寄一封確認信到你的Gmail信箱裡，在按下驗證連結就設定完成了

![完成Gmail SMTP Server設定](https://media.secologies.com/adding-email-done.webp)

![驗證Gmail SMTP Server設定](https://media.secologies.com/confirm-gmail-smtp-settings.webp)
...
![Cloudflare驗證Gmail SMTP Server的DNS紀錄](https://media.secologies.com/cloudflare-gmail-dns.webp)

最後回Gmail帳號的設定，在回覆時可以調整成寄件者指定的信箱作為回信，也可以調整預設回覆的信箱

![Gmail SMTP設定回信的信箱位址](https://media.secologies.com/replying-setting-2.webp)

## 測試
最後我們來測試一下有沒有設定成功，利用其他的信箱寄件給Cloudflare Domain的位址一封電子郵件

![從protonmail寄送到我的客製化信箱](https://media.secologies.com/sending-test.webp)

![Gmail確實有收到信件，郵件位址也正確](https://media.secologies.com/receive-test.webp)

![用Gmail回信，protonmail確實收到來自hshuang@hshuang.blog的信件](https://media.secologies.com/reply-test.webp)
